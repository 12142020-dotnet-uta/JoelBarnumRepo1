...
namespace RpsGame_NoDb
{
    public class RpsGameRepositoryLayer
    {
        ...
        //List<Player> matches = DbContext.players.ToList();
        DbSet<Round> rounds = DbContext.rounds;
        //List<Player> rounds = DbContext.players.ToList();
        ...
        public bool SaveMatch(Match match)
        {
            ...
            if (!matches.Any(x => x.MatchId == match.MatchId))//!matches.Any(x => x.matchId == match.MatchId)
            {
                matches.Add(match);
                 DbContext.SaveChanges();
                return true;
